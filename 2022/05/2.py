...
def moveStacks(origin, destination, amount):
    movedStack = []
    for i in range(amount):
        movedStack.insert(0, origin.pop())
    destination.extend(movedStack)

stacks = getStacks(vis)
for instruction in instructions:
    amount, originIdx, destinationIdx = [int(x) for x in re.findall(r'\d+', instruction)]
    moveStacks(stacks[originIdx - 1], stacks[destinationIdx - 1], amount)
result = ""
for stack in stacks:
    result += stack[-1]
print(result)